...
## Implemented on srs-v1 — September 2026

- Three alternating names rows now follow the hero. Existing `workedWith` data is split across rows; duplicated visual tracks are hidden from assistive technology, which gets one semantic list. A pause control, hover/focus pause, visibility observer and reduced-motion static layout are included. No animation library was added.
- The first four `featuredWork` entries form the desktop accordion. Equal resting widths transition to approximately 55% / 15% / 15% / 15%, without changing row height. Keyboard focus takes precedence over hover. The remaining projects are retained beneath it.
- Below 900px, cards remain wide and full-colour in a horizontally scrollable, snapping rail with previous/next controls. No hover is required. Reduced motion removes transitions.
- `ProjectAccordion` accepts optional `poster` and `href` fields. Only add approved project media and real destinations to `content/srs/v1.ts`. The current repository has no portfolio posters, so the component explicitly displays “Image coming soon”. Colour reveal is currently visible on the gradient fallback. Video preview loading/playback remains future work when real media is supplied; no speculative video downloads or players are included.
- The password gate has restored layout styles, a visible label and submit button, error announcement and focus styles. The decorative canvas is aria-hidden and ignores pointer events; the server-side password check is unchanged.

Validation: local mouse password entry, incorrect-password error, successful unlock, desktop keyboard expansion, and 390px carousel navigation with no page overflow. Production build and lint are checked before committing. Visual media quality cannot be assessed until real posters/clips are supplied.
